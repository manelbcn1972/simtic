...
Deno.serve(async (request) => {
  ...
  await supabase.from("lead_signal_enrichments").upsert(["apollo", "trace", "predictleads", "crunchbase"].map(provider => ({ event_id: saved.id, provider, status: "pending" })), { onConflict: "event_id,provider" });
